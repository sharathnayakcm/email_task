  ...
  $.TokenList = function (input, url_or_data, settings) {
    ...
    if(li_data && li_data.length) {
      $.each(li_data, function (index, value) {
        insert_token(value,'prepolulated');
        checkTokenLimit();
      });
    }
    ...
    function insert_token(item,check_prepulated) {
    ...
    function add_token (item) {
      ...
      if(token_count > 0 && settings.preventDuplicates) {
        ...
        if(found_existing_token) {
          $('#msg').html("<span class='err_msg'>Member already exist</span>");
          notify();
    ...
    function delete_token (token) {
      if(token.html().search(/prepolulated/i) != -1 && $('#moderator_cug_user').val() != $('#logged_in_user').val()){
        alert("you should be moderator to delete the user");
        return false;
      }
